...
export async function linkedin(req, res) {
    ...
    const emailApiUrl = config.linkedInLogin.emailApiUrl;
    var params = {
        code: req.query.code,
    ...
    try {
        if (req.query.error == 'user_cancelled_authorize') {
            ...
            const token = await getAccessToken(params, accessTokenUrl);
            const getProfile = await getPeople(token.access_token, peopleApiUrl);
            const getEmailAddress = await getPeople(token.access_token, emailApiUrl);

            let profile = {};
            profile['id'] = getProfile.id;
            profile['firstName'] = getProfile.firstName.localized.en_US;
            profile['lastName'] = getProfile.lastName.localized.en_US;
            profile['pictureUrl'] = null;

            if (getEmailAddress && getEmailAddress.elements) {
                profile['emailAddress'] = getEmailAddress.elements[0]['handle~'].emailAddres;
            }
