...
let sqlQueries = {
	...
	productViewAndReviewCategoryCount: function(queryObj, productQueryObj) {
		if (queryObj.status && productQueryObj.vendor_id) {
			...
			category RIGHT OUTER JOIN sub_category on category.id = sub_category.category_id
			LEFT OUTER JOIN (product JOIN vendor ON product.vendor_id = vendor.id AND vendor.status = 1 JOIN vendor_plan ON vendor.id = vendor_plan.vendor_id AND vendor_plan.status = 1 AND vendor_plan.start_date <= '`+new Date().toISOString().slice(0,10)+`' AND vendor_plan.end_date >= '`+new Date().toISOString().slice(0,10)+`') on sub_category.id = product.sub_category_id and product.status=(` + queryObj.status + `) and product.vendor_id=(` + productQueryObj.vendor_id + `)
	...
	vendorFilterCatogoryCount: function(params) {
		if (params.vendor_id && params.marketplace_id && !params.marketplace_type_id) {
			...
			category RIGHT OUTER JOIN sub_category on category.id = sub_category.category_id
			LEFT OUTER JOIN (product JOIN vendor ON product.vendor_id = vendor.id AND vendor.status = 1 JOIN vendor_plan ON vendor.id = vendor_plan.vendor_id AND vendor_plan.status = 1 AND vendor_plan.start_date <= '`+new Date().toISOString().slice(0,10)+`' AND vendor_plan.end_date >= '`+new Date().toISOString().slice(0,10)+`')  on sub_category.id = product.sub_category_id and product.status=(` + params.status + `) and product.marketplace_id=(` + params.marketplace_id + `) and product.vendor_id=(` + params.vendor_id + `)
			GROUP BY sub_category.id ORDER by category.id`;
			return query;
