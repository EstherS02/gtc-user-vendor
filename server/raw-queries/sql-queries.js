...
let sqlQueries = {
	...
	LEFT JOIN vendor_plan on vendor.id = vendor_plan.vendor_id 
    ...
    LEFT JOIN product_ratings ON product.id = product_ratings.product_id where product.status=1 AND vendor.status=1 AND vendor_plan.status=1 AND vendor_plan.start_date <= '` + new Date().toISOString().slice(0, 10) + `'  AND vendor_plan.end_date >= '` + new Date().toISOString().slice(0, 10) + `' 
    HAVING
        distance < 300
    ORDER BY
        product.product_name`;
	...
	vendorShippingLocation: function(params) {
		...
		product.vendor_id,
		product.product_name,
		vendor_shipping_location.country_id
	FROM
		cart
